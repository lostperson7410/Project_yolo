...
def findObjects(outputs,img,id):
    ...
    for i in indices:
        ...
        if classIds == [2]:
            ...
            CNimg = cv.imread("data/pic"+str(id)+".jpg",cv.IMREAD_COLOR)
            #findlicenes(CNimg)
...
def findlicenes(crop):
    ...
    xl,yl,wl,hl = cv.boundingRect(largest_rectangle[1])
    imgRoi = crop[yl:yl+hl,xl:xl+wl]
    cv.imshow("Ls",imgRoi)


def getImg(img,id):
      cv.imwrite("data/pic"+str(id)+".jpg",img)
      id = id+1
